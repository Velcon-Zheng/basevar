...
cdef extern from "em.h" nogil:
    void cEM(double *init_allele_freq, double *ind_allele_likelihood, double *marginal_likelihood, double *expect_allele_prob, int nsample, int ntype, int iter_num, double epsilon) nogil
    
cdef extern from "ranksum.h" nogil:
    double cRankSums(double *x, int n1, double *y, int n2)

cdef extern from "kfunc.h" nogil:
