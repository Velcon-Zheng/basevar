...
def EM(double[::1] init_allele_freq, double[:, ::1] ind_allele_likelihood, int iter_num=100, double epsilon=0.001):
    ...
    cEM(& init_allele_freq[0], & ind_allele_likelihood[0, 0], & marginal_likelihood[0], & expect_allele_prob[0], nsample, ntype, iter_num, epsilon)
...
def ref_vs_alt_ranksumtest(ref_base, alt_base, data):
    ...
    cdef double z = cRankSums( & x[0], nx, & y[0], ny)
